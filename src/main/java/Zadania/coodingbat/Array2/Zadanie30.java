package Zadania.coodingbat.Array2;

/*Return a version of the given array where all the 10's have been removed.
The remaining elements should shift left towards the start of the array as needed,
and the empty spaces a the end of the array should be 0. So {1, 10, 10, 2} yields {1, 2, 0, 0}.
You may modify and return the given array or make a new array.
        withoutTen([1, 10, 10, 2]) → [1, 2, 0, 0]
        withoutTen([10, 2, 10]) → [2, 0, 0]
        withoutTen([1, 99, 10]) → [1, 99, 0]*/

import java.util.ArrayList;
import java.util.List;

public class Zadanie30 {
    public static void main(String[] args) {

        int[] tablica1 = {1, 10, 10, 2};
        int[] tablica2 = {10, 2, 10};
        int[] tablica3 = {1, 99, 10};

        wyswietlacz(withoutTen(tablica1));
        wyswietlacz(withoutTen(tablica2));
        wyswietlacz(withoutTen(tablica3));

    }
    public static int[] withoutTen(int[] nums) {
        List<Integer> result = new ArrayList<>();
        for(int i=0; i< nums.length;i++){
            if(nums[i]!=10){
                result.add(nums[i]);
            }
        }
        for(int i=0; i< nums.length;i++){
            if(nums[i]==10){
                result.add(0);
            }
        }
        return result.stream().mapToInt(Integer::intValue).toArray();
    }

    public static void wyswietlacz(int[] tab) {
        for (int element : tab) {
            System.out.print(element + " ");
        }
        System.out.println();
    }

}
